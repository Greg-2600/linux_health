...
def render_report_text(system: SystemInfo, checks: Iterable[CheckResult], ports: Iterable[PortStatus], detailed: DetailedSecurityInfo | None = None) -> str:
    ...
    if detailed:
        ...
        if detailed.rootkit_scan is not None:
            ...
            if len(detailed.rootkit_scan.splitlines()) > 30:
                lines.append(f"  ... (showing first 30 of {len(detailed.rootkit_scan.splitlines())} lines)")
            lines.append("")
        ...
        if detailed.unused_packages is not None:
            ...
            if len(detailed.unused_packages.splitlines()) > 30:
                lines.append(f"  ... (showing first 30 of {len(detailed.unused_packages.splitlines())} lines)")
            lines.append("")
        ...
        if len(detailed.suid_binaries.splitlines()) > 15:
            lines.append(f"  ... and {len(detailed.suid_binaries.splitlines()) - 15} more")
        lines.append("")
    ...
    lines.append("-" * 80)
    lines.append("  • ps -eo pid,cmd,%cpu,%mem --sort=-%cpu | head     (inspect top processes)")
    lines.append("  • sudo du -xhd1 / | sort -h                        (disk usage by directory)")
    lines.append("  • sudo apt-get update && apt-get upgrade           (apply security updates)")
    lines.append("  • sudo ufw status verbose                          (review firewall rules)")
    lines.append("  • sudo sshd -T                                     (verify SSH config)")
    lines.append("")
...
def render_report(system: SystemInfo, checks: Iterable[CheckResult], ports: Iterable[PortStatus], detailed: DetailedSecurityInfo | None = None) -> str:
    checks_list = list(checks)
    ports_list = list(ports)
    ts = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    ...
    open_ports = [p.port for p in ports_list if p.open]
    lines.append(f"- Open ports (scanned): {len(open_ports)} -> {', '.join(map(str, open_ports)) if open_ports else 'none'}")
    lines.append("")
    ...
    open_only = [p for p in ports_list if p.open]
    if not open_only:
        lines.append("| none | closed/filtered | No open ports found (all scanned ports were closed/filtered) |")
    else:
        for port in open_only:
    ...
    lines.append("- Inspect top CPU/mem: `ps -eo pid,cmd,%cpu,%mem --sort=-%cpu | head`")
    lines.append("- Disk usage by dir: `sudo du -xhd1 / | sort -h`")
    lines.append("- Pending updates (Debian/Ubuntu): `sudo apt-get update && sudo apt-get upgrade`")
    lines.append("- Firewall (ufw): `sudo ufw status verbose`")
    lines.append("- SSH hardening: edit `/etc/ssh/sshd_config` then `sudo systemctl reload sshd`")
    lines.append("")
    lines.append("Notes: Port scan is TCP connect scan on provided/common ports; results may be filtered by firewalls.")
    return "\n".join(lines)
